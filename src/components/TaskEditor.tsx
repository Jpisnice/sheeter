...
export function TaskEditor({
  ...
  autoFocus?: boolean
  /** From Settings → Preferences; defaults to 7:30–8:00. */
  dayMinMins?: number
  dayMaxMins?: number
  ...
  // auto so the backend can redistribute to fit the configured daily window.
  ...
  const anyLocked = useMemo(() => {
    return activeRows.some(
      (r) =>
        lockedIds.has(r.id) && r.hours.trim() !== '' && r.label.trim() !== '',
    )
  ...
  const validateTotals = (
    ...
    if (unlockedCount === 0) {
      if (lockedMins < lo) {
        return `The day totals ${formatHMM(lockedMins)}, which is below your minimum (${formatHMM(lo)}). Bump one or more tasks up, or clear a value so it auto-fills.`
      }
      if (lockedMins > hi) {
        return `The day totals ${formatHMM(lockedMins)}, which is above your maximum (${formatHMM(hi)}). Trim one or more tasks down.`
      }
      return null
    }
    if (lockedMins + unlockedCount * 15 > hi) {
      const ts = unlockedCount === 1 ? 'task' : 'tasks'
      return `Your locked hours (${formatHMM(lockedMins)}) leave no room for ${unlockedCount} auto ${ts} within ${formatHMM(hi)} max. Reduce a locked value or remove a task.`
    }
    return null
  }
  ...
  const willRedistribute =
    ...
    activeRows.some(
      (r) =>
        r.label.trim() !== '' && r.hours.trim() !== '' && !lockedIds.has(r.id),
    )
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-2">
        {rows.map((r, i) => {
          const dim = anyLocked && !lockedIds.has(r.id) && r.hours.trim() !== ''
          ...
          return (
            <div
              ...
              style={isLeaving ? undefined : { animationDelay: `${i * 30}ms` }}
              ...
              <input
                ...
                title={
                  dim
                    ...
                    : `Enter hours as 3, 230 (= 2:30), 500 (= 5:00), or 1:30. Max ${formatHMM(Math.min(dayMaxMins, 480))} per task.`
                }
      ...
      {willRedistribute ? (
        <p className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#8b8780]">
          Other tasks will auto-adjust to {formatHMM(dayMinMins)}–
          {formatHMM(dayMaxMins)}
        </p>
      ) : null}
